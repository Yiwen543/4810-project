...
"""# KMean Segementation"""
def kmeans_segmentation(image, k=4, spatial_weight=0.2):
    """
    改进版 K-Means：
...
def region_growing(image, kmeans_mask=None, threshold=15):
    """
    改进版 Region Growing:
    ...
    if kmeans_mask is not None and np.sum(kmeans_mask) > 0:
        ...
        num_labels, _, stats, centroids = cv2.connectedComponentsWithStats(kmeans_mask, connectivity=8)
...
def visualize_results(original, mask, segmented, method_name, metrics=None, save_path=None):
    ...
    if metrics:
        title_text = (f'{method_name}\n'
                      f'Dice: {metrics["dice"]:.4f} | IoU: {metrics["iou"]:.4f} | Acc: {metrics["accuracy"]:.4f}')
...
rg = region_growing(image, kmeans_mask=kmeans)
